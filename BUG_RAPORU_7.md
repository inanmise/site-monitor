...
- ~~Ayarlar sekmesi `user === 'admin'` sabit kapısı: müdür/global admin hesapları retention ekranını göremez — ayrı bir karar konusu, bu turda dokunulmadı.~~ **Kapatıldı (2026-09-10, sonraki tur):** kapı `global_admin` bayrağına çevrildi (App.jsx + Nav.jsx, `Nav.test.jsx` ile pinli). Kapsamlı müdür (AD ADMIN) bilinçli olarak dışarıda: dokuz ayar denetleyicisi `requireNotScopedAdmin` ile reddediyor (v20.50.29 güvenlik kararı); UI backend ile aynı hizada.
- ~~Prod Helm değerleri 20/50/5000 bir sonraki `helm upgrade` ile devreye girer.~~ **Helm'e gerek kalmadı (2026-09-10):** `site.monitor.executor.core-size/max-size/queue-capacity` artık `AppSettingsCatalog`'da (grup "Görev Havuzu"); kaydedince `AppSettingsChangedEvent` → `ExecutorTuningService` executor'ı canlı boyutlar (`TunableThreadPoolTaskExecutor` + `ResizableCapacityQueue`). Env değerleri yalnız açılış; DB override yeniden başlatmada `ApplicationReadyEvent`'te geri uygulanır.
