...
describe('RenewalAdvice — yeniden tasarım', () => {
  ...
  it('sayfalama: 34 öneri → 25 kart + sayfalama çubuğu; tablo görünümünde satırlar ve takım rozeti; ulaşılamayanda Tanıla', async () => {
    ...
    expect(row3.querySelector('.team-badge').textContent).toContain('Takım B')
  ...
  it('neden / takım / etiket süzgeçleri ve arama (veren adında da eşleşir)', async () => {
    ...
    const open = (re) => fireEvent.mouseDown([...bar.querySelectorAll('.ss-trigger')].find((b) => re.test(b.textContent)))
    const pick = (re) => fireEvent.mouseDown([...bar.querySelectorAll('.ss-option')].find((o) => re.test(o.textContent)))
