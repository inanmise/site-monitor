...
describe('PushLogView', () => {
  ...
  it('KPI Engellendi → status=BLOCKED; seviye kırılımı → level; alıcı satırı → username çipi; filtreleri temizle', async () => {
    ...
    fireEvent.click(within(document.querySelector('.sml-grid')).getByRole('button', { name: 'HIGH' }))
    await waitFor(() => expect(pushLog.search).toHaveBeenLastCalledWith(expect.objectContaining({ level: 'HIGH' })))
    fireEvent.click(within(document.querySelector('.sml-grid')).getByRole('button', { name: /Üç Kullanıcı/ }))
