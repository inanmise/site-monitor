    ...
    <ModalShell open onClose={onLater} size="sm" icon={whatsNew ? Sparkles : Compass}
      ...
      footer={(
        <div className="tour-welcome-actions">
          ...
          <button type="button" className="btn btn-secondary" onClick={onLater}>{t('tour.later')}</button>
          <button type="button" className="btn btn-primary" onClick={onStart} autoFocus>{t(whatsNew ? 'tour.newStart' : 'tour.start')}</button>
        </div>
      )}>
