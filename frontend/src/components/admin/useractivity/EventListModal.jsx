            ...
            <tbody>{pager.pageItems.map((r, i) => {
              ...
              return (
                <tr key={r.id ?? `${r.time}-${i}`} className={`uact-row${!ok && r.outcome ? ' evl-row--bad' : ''}`}>
                  ...
                  <td data-label={t('uact.colUser')}>{r.actor ? <button type="button" className="uact-link udir-name" onClick={() => onUser?.({ username: r.actor, ...r })}><UserBadge username={r.actor} userId={r.user_id} displayName={r.display_name} inline size="sm" /></button> : '—'}{r.system_role && <span className="udir-sub">{r.system_role}{r.event_type && !/^LOGIN(_SUCCESS)?$/.test(r.event_type) ? ` · ${r.event_type}` : ''}</span>}</td>
