 ...
 */
export default function TeamBadge({ teamId, teamName, size = 12, className = '', onOpen, static: forceStatic = false, title }) {
  const t = useT()
  ...
  return (
    <>
      <button type="button" className={`team-badge${className ? ' ' + className : ''}`}
        onClick={openIt} title={title || t('team.openMembers', name)} aria-label={t('team.openMembers', name)}>
