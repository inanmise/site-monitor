<#
.SYNOPSIS
  Post-deploy smoke test for CertMonitor. Probes the most load-bearing endpoints
  to verify the app is alive, the auth gate is honoured, and the public health
  surface returns the expected shape.

.PARAMETER BaseUrl
  Root URL of the running CertMonitor instance. Defaults to http://localhost:8080.
...
Write-Host ""
Write-Host "CertMonitor smoke test against $BaseUrl" -ForegroundColor Cyan
Write-Host ("=" * 70)
