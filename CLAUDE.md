...
**Recipient routing (team-only vs escalation)** — a single rule, `isTeamOnly(alertType)`, governs three paths (initial alert `processConfirmedOutage`, resolution `sendResolutionNotification`, manual resend `reNotify`) and MUST stay consistent across them:
- Free-form monitoring + domain alerts (keyword/ping/http/`TYPE_DOMAIN_EXPIRY`/all `DOMAINMON_*`) are **team-only**: recipients = the team's email(s), resolved from `AlertEvent.teamId` (stamped at alarm creation) — **not** the certificate inventory. No manager/global escalation contact is added. Standalone domain monitors aren't in `certificate_inventory`, so resolving their team from inventory silently drops to global contacts (the "resend went to the müdür, not the team" bug — fixed by routing all three paths through `isTeamOnly` + `event.getTeamId()`).
...
- **`AuditService`** (`@Async`) — writes every LOGIN / LOGIN_FAILED / LOGOUT / CRUD / SETTING event to `audit_logs` with IP+geo and a CSV `anomaly_flags` column (OFF_HOURS / UNUSUAL_IP / GEO_VELOCITY / BRUTE_FORCE / RATE_LIMITED). Detection windows are `cert.monitor.audit.*` props.
