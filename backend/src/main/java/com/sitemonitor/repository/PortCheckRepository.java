...
public interface PortCheckRepository extends JpaRepository<PortCheck, Long> {
    ...
    @Query("SELECT SUBSTRING(c.checkedAt,1,:len), COUNT(c), SUM(CASE WHEN c.open = false THEN 1L ELSE 0L END) "
         + "FROM PortCheck c WHERE c.monitorId = :id AND c.checkedAt >= :from AND c.checkedAt <= :to "
