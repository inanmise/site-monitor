...
public interface DomainCheckRepository extends JpaRepository<DomainCheck, Long> {
    ...
    @Query("SELECT SUBSTRING(c.checkedAt,1,:len), COUNT(c), SUM(CASE WHEN c.status <> 'OK' THEN 1L ELSE 0L END) "
         + "FROM DomainCheck c WHERE c.monitorId = :id AND c.checkedAt >= :from AND c.checkedAt <= :to "
