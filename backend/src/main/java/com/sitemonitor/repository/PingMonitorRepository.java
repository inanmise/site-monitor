...
public interface PingMonitorRepository extends JpaRepository<PingMonitor, Long> {
    ...
    @Modifying
    @Query("UPDATE PingMonitor m SET m.groupName = :newName WHERE LOWER(m.groupName) = LOWER(:oldName) AND ((:teamId IS NULL AND m.teamId IS NULL) OR m.teamId = :teamId)")
    ...
    java.util.List<PingMonitor> findByNotificationGroupId(Long notificationGroupId);
