...
public class CertificateCheckerService {
    ...
    @Value("${site.monitor.check.retry-on-transient:true}")
    private boolean retryOnTransient;

    @Value("${site.monitor.check.retry-delay-ms:1000}")
    private long retryDelayMs;

    @Value("${site.monitor.check.max-attempts:2}")
    ...
    @Value("${site.monitor.check.retry-fallback:true}")
    ...
    @Value("${site.monitor.check.tls-mode:browser}")
    ...
    @Value("${site.monitor.proxy.auto-fallback:false}") private boolean autoProxyFallback;
    ...
    public X509Certificate[] captureProxyChain(String host, int port) throws IOException {
        if (proxyHost == null || proxyHost.isBlank() || proxyPort <= 0) {
            throw new IOException("Proxy yapılandırılmamış (site.monitor.proxy.host/port boş) — CA zinciri yalnız proxy üzerinden yakalanır");
        }
        int timeoutSec = 8;
