 ...
 * → önceki haftayla gerçek delta. Ağırlıklar {@code cert.monitor.weekly.score.weight-*} anahtarlarından CANLI okunur
...
public class WeeklyScoreCalculator {
    ...
    public int score(ScoreInputs in) {
        double wCrit   = appSettings.getDouble("cert.monitor.weekly.score.weight-critical", 8.0);
        double wExp    = appSettings.getDouble("cert.monitor.weekly.score.weight-expiring", 2.0);
        double wWeak   = appSettings.getDouble("cert.monitor.weekly.score.weight-weak-algo", 3.0);
        double wUptime = appSettings.getDouble("cert.monitor.weekly.score.weight-uptime", 0.5);
