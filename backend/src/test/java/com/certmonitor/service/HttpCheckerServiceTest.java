 ...
 * ({@code cert.monitor.trust.ca-bundle-pem}) VE CA otomatik sabitleme (auto-pin / TOFU,
...
class HttpCheckerServiceTest {
    ...
    static void startSelfSignedHttpsServer() throws Exception {
        ...
        serverCert = selfSigned(kp, "CN=localhost, O=CertMonitor Test");
        serverCertPem = toPem(serverCert);
        KeyPair staleKp = generateKeyPair();
        staleCert = selfSigned(staleKp, "CN=localhost, O=CertMonitor Test Old");
