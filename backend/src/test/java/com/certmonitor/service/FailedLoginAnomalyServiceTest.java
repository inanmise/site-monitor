...
class FailedLoginAnomalyServiceTest {
    ...
    void setup() {
        ...
        lenient().when(appSettings.getInt(eq("cert.monitor.failed-login.window-minutes"), anyInt())).thenReturn(10);
        lenient().when(appSettings.getInt(eq("cert.monitor.failed-login.threshold-total"), anyInt())).thenReturn(20);
        lenient().when(appSettings.getInt(eq("cert.monitor.failed-login.threshold-per-account"), anyInt())).thenReturn(5);
        lenient().when(appSettings.getInt(eq("cert.monitor.failed-login.threshold-per-ip"), anyInt())).thenReturn(15);
        lenient().when(appSettings.getInt(eq("cert.monitor.failed-login.threshold-distinct-users-per-ip"), anyInt())).thenReturn(5);
        lenient().when(appSettings.getInt(eq("cert.monitor.failed-login.threshold-distinct-ips-per-account"), anyInt())).thenReturn(5);
        lenient().when(appSettings.getInt(eq("cert.monitor.failed-login.baseline-hours"), anyInt())).thenReturn(24);
        lenient().when(appSettings.getInt(eq("cert.monitor.failed-login.relative-floor"), anyInt())).thenReturn(8);
        lenient().when(appSettings.getDouble(eq("cert.monitor.failed-login.relative-multiplier"), anyDouble())).thenReturn(3.0);
