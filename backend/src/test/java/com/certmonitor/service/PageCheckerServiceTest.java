...
class PageCheckerServiceTest {
    ...
    void setUp() throws IOException {
        ...
        checker = new PageCheckerService(guard);
        checker.init();
    ...
    void allGood_ok() {
        var r = checker.check(base + "/ok", "SINGLE_PAGE", 5000, 2000, 5, null, 2, 50);
    ...
    void brokenImg_degraded() {
        var r = checker.check(base + "/broken", "SINGLE_PAGE", 5000, 2000, 5, null, 2, 50);
    ...
    void mainError_down() {
        var r = checker.check(base + "/down500", "SINGLE_PAGE", 5000, 2000, 5, null, 2, 50);
    ...
    void redirectChain_followed() {
        var r = checker.check(base + "/redirect", "SINGLE_PAGE", 5000, 2000, 5, null, 2, 50);
        assertThat(r.status()).isEqualTo("OK");
        assertThat(r.mainReachable()).isTrue();
    }
    ...
    void headUnsupported_fallsToGet() {
        var r = checker.check(base + "/headonly", "SINGLE_PAGE", 5000, 2000, 5, null, 2, 50);
        assertThat(r.status()).isEqualTo("OK");
        assertThat(r.issues()).isEmpty();
    }
    ...
    void excludePattern_skipsResource() {
        var r = checker.check(base + "/excludable", "SINGLE_PAGE", 5000, 2000, 5, "/ads/", 2, 50);
        assertThat(r.status()).isEqualTo("OK");
        assertThat(r.issues()).isEmpty();
    }
    ...
    void ssrfResource_blocked() {
        var r = checker.check(base + "/ssrf", "SINGLE_PAGE", 5000, 2000, 5, null, 2, 50);
        assertThat(r.status()).isEqualTo("DEGRADED");
        assertThat(r.issues()).anySatisfy(i -> {
            assertThat(i.issueType()).isEqualTo("BROKEN");
    ...
    void crawl_respectsDepthAndRobots() {
        var r = checker.check(base + "/site", "SITE_CRAWL", 5000, 2000, 5, null, 1, 10);
