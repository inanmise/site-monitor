...
import com.sitemonitor.service.FailedLoginAnomalyService.RuleHit;
import com.sitemonitor.service.FailedLoginAnomalyIncidentService.Window;
...
class FailedLoginAnomalyIncidentServiceTest {
    ...
    @BeforeEach
    void setup() {
        when(appSettings.getInt(eq("site.monitor.failed-login.window-minutes"), anyInt())).thenReturn(10);
        when(appSettings.getInt(eq("site.monitor.failed-login.catchup-cap-minutes"), anyInt())).thenReturn(60);
        when(appSettings.getInt(eq("site.monitor.failed-login.cooldown-minutes"), anyInt())).thenReturn(60);
        when(appSettings.getBoolean(eq("site.monitor.failed-login.resolved-email-enabled"), anyBoolean())).thenReturn(true);
        when(appSettings.getCsv(eq("site.monitor.failed-login.alert-recipients"), anyString())).thenReturn(List.of());
        when(appSettings.getString(eq("site.monitor.system-admin.email"), any())).thenReturn("admin@x");
    ...
    void anomalyEnds_resolvedEmailDisabled_noEmail() {
        when(appSettings.getBoolean(eq("site.monitor.failed-login.resolved-email-enabled"), anyBoolean())).thenReturn(false);
        when(incidentRepo.findFirstByResolvedFalseOrderByOpenedAtDesc())
                .thenReturn(Optional.of(openIncident("GLOBAL_VOLUME", 20, NOW_ISO)));
        service.handle(quiet(), NOW);
        verify(incidentRepo).save(argThat(LoginAnomalyIncident::isResolved));
