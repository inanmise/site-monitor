...
class LoginIssueMailServiceTest {
    ...
    void setup() {
        ...
        when(appSettings.getBoolean(eq("site.monitor.login-issues.force-email"), anyBoolean())).thenReturn(true);
    }
    ...
    void dispatchResolved_forceFalseFromConfig_recipientArrangement() {
        when(appSettings.getBoolean(eq("site.monitor.login-issues.force-email"), anyBoolean())).thenReturn(false);
