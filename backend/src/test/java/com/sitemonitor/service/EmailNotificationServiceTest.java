...
class EmailNotificationServiceTest {
    ...
    // Logback yakalayıcılar: mail TRACE detayları "com.sitemonitor.mail"; operasyonel
    ...
    void setUp() {
        ...
        mailLogger = (Logger) LoggerFactory.getLogger("com.sitemonitor.mail");
    ...
    private SmtpSettings settingsEnabledFull() {
        ...
        s.setUsername("svc@sitemonitor.com");
    ...
    void sendAlert_rich_emailDisabled_returnsSkipped() {
        String result = service.sendAlert(
                "to@test.com", "[SiteMonitor UYARI] test.com — 25 gün kaldı",
    ...
    void sendResolutionAlert_emailDisabled_returnsSkipped() {
        String result = service.sendResolutionAlert(
                "to@test.com",
                "[SiteMonitor ✅ ÇÖZÜLDÜ] test.com — Sertifika Süre Bitişi sorunu giderildi",
    ...
    void buildAlertEmailHtml_expiry_executive() {
        String html = service.buildAlertEmailHtml(
                "[SiteMonitor] ORTA · x.com · Sertifika 25 gün içinde doluyor", "msg",
    ...
    void buildAlertEmailHtml_accessibility_containsOutageDetails() {
        ...
        String html = service.buildAlertEmailHtml(
                "[SiteMonitor KRİTİK] down.example.com — Erişim Kesintisi",
    ...
    void buildAlertEmailHtml_portDown_containsPortDetails() {
        ...
        String html = service.buildAlertEmailHtml(
                "[SiteMonitor KRİTİK] down.example.com — Port Kesintisi",
    ...
    void retryExhausted_writesBackFailed() throws Exception {
        String subject = "[SiteMonitor YÜKSEK] kartfree.com — DNS";
        when(settingsService.getOrDefaults()).thenReturn(settings(true));
        when(smtpMailService.currentSender()).thenReturn(sender);
    ...
    void sendAlert_revokedType_emailEnabled_sends() throws Exception {
        ...
        String result = service.sendAlert(
                "to@test.com", "[SiteMonitor KRİTİK] revoked.com — İptal Edildi",
    ...
    // ── Mail TRACE logging + hata stack izi (com.sitemonitor.mail) ────────────────
    ...
    @Test
    @DisplayName("TRACE kapalı (INFO): com.sitemonitor.mail'e hiç TRACE satırı düşmez")
